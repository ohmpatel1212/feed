...
export async function getFeedPreviewPosts(
  ...
  if (feed.subqueries.length === 0) return [];
  ...
  try {
    ...
    if (feed.rerank_prompt.trim().length > 0 && hits.length > 0) {
      rerankAttempted = true;
      try {
        // Listwise rerank latency scales steeply with candidate count.
        // Cap the input at RERANK_INPUT_MAX top-by-vector candidates so a
        // refresh stays in the few-seconds range even with a generous N.
        const RERANK_INPUT_MAX = 80;
        const inputHits =
          hits.length > RERANK_INPUT_MAX
            ? [...hits]
                .sort((a, b) => b.vector_score - a.vector_score)
                .slice(0, RERANK_INPUT_MAX)
            : hits;
        const r = await rerank({
          ...
          candidates: inputHits,
          topK: limit,
          systemPrompt: feed.rerank_prompt,
        });
        msRerank = r.ms_rerank;
        // r.kept[].i indexes into inputHits, not hits. Translate back.
        const inputToHit = new Map<number, number>();
        for (let j = 0; j < inputHits.length; j++) {
          const origIdx = hits.indexOf(inputHits[j]);
          if (origIdx >= 0) inputToHit.set(j, origIdx);
        }
        rerankByIndex = new Map();
        orderedHits = [];
        for (const k of r.kept) {
          const origIdx = inputToHit.get(k.i);
          if (origIdx === undefined) continue;
          rerankByIndex.set(origIdx, { score: k.score, reason: k.reason });
          orderedHits.push(hits[origIdx]);
        }
      } catch (e) {
