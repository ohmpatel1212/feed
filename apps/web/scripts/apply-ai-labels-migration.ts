/**
 * Apply sql/008_ai_image_labels.sql to the feed-db Postgres instance.
 *
 ...
 * never printed.
 *
 * Run with: `npx tsx scripts/apply-ai-labels-migration.ts`
 */
...
async function main() {
  ...
  const sqlPath = join(here, "..", "sql", "008_ai_image_labels.sql");
  ...
  const cols = await pool.query(`
    SELECT column_name, data_type, is_nullable, column_default
     ...
     WHERE table_name = 'ai_image_labels'
     ORDER BY ordinal_position`);
  console.log("\nai_image_labels columns:");
